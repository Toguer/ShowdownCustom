...
export const commands: Chat.ChatCommands = {
	...
	monstats: {
		help: [`/monstats [p1|p2] - Muestra stats base y efectivos (con objetos/habilidades) de los Pokémon activos.`],
		async handler(target, room, user) {
		  this.checkCan('console');
		  if (!room || !room.battle) return this.errorReply(`Este comando solo funciona en una sala de batalla.`);
		  ...
		  if (t === 'p1' || t === 'p2') {
			...
			const p1name = room.battle.p1?.name;
			const p2name = room.battle.p2?.name;
			if (p2name && p2name === user.name) side = 'p2';
			else if (p1name && p1name === user.name) side = 'p1';
		  }
	  
		  let data: any;
		  try {
			data = await (room.battle as any).getDebug(side);
		  } catch (e: any) {
			return this.errorReply(`Error leyendo debug: ${e?.message || e}`);
		  }
		  if (!data || data.error) return this.errorReply(data?.error || `No se pudo obtener debug.`);
	  
		  let html = `<b>Stats (${side})</b><br/>`;
		  for (const mon of data.actives || []) {
			const boosts = mon.boosts || {};
			const boostStr = `atk:${boosts.atk ?? 0} def:${boosts.def ?? 0} spa:${boosts.spa ?? 0} spd:${boosts.spd ?? 0} spe:${boosts.spe ?? 0}`;
			html += `
			  <div style="margin:6px 0;padding:6px;border:1px solid #AAA">
				<b>${mon.name}</b> (${mon.species})<br/>
				Ability: <code>${mon.ability || '(none)'}</code> — Item: <code>${mon.item || '(none)'}</code><br/>
				Boosts: <code>${boostStr}</code><br/>
				ATK <code>${mon.atkBase}</code> → <b>${mon.atk}</b> |
				DEF <code>${mon.defBase}</code> → <b>${mon.def}</b> |
				SpA <code>${mon.spaBase}</code> → <b>${mon.spa}</b> |
				SpD <code>${mon.spdBase}</code> → <b>${mon.spd}</b> |
				SPE <code>${mon.speBase}</code> → <b>${mon.spe}</b>
			  </div>`;
		  }
