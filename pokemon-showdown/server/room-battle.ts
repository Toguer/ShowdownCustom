...
export class RoomBattle extends RoomGame<RoomBattlePlayer> {
	...
	async getDebug(side: SideID) {
		void this.stream.write(`>requestdebug ${side}`);
		const debugPromise = new Promise<string[]>((resolve, reject) => {
			if (!this.dataResolvers) this.dataResolvers = [];
			this.dataResolvers.push([resolve, reject]);
		});
		const result = await debugPromise;
		// result[0] será el JSON
...
export class RoomBattleStream extends BattleStream {
	...
	override _write(chunk: string) {
		...
		for (const line of lines) {
			...
			if (line.startsWith('>requestdebug ')) {
				const sideid = line.slice('>requestdebug '.length).trim() as SideID;
				const side = this.battle?.sides.find(s => s.id === sideid);
				const payload = side ? {
					side: side.id,
					actives: side.active.filter(Boolean).map(p => ({
						name: p.name,
						...
						// BASE (sin objetos/habilidades)
						atkBase: p.storedStats?.atk,
						defBase: p.storedStats?.def,
						spaBase: p.storedStats?.spa,
						spdBase: p.storedStats?.spd,
						speBase: p.storedStats?.spe,
					  
						// EFECTIVOS (con objetos/habilidades)
						atk: p.getStat('atk', false, false),
						def: p.getStat('def', false, false),
						spa: p.getStat('spa', false, false),
						spd: p.getStat('spd', false, false),
						spe: p.getStat('spe', false, false),
					})),
				} : {error: `Side ${sideid} not found`};
		...
		try {
			if (forwarded.length) this._writeLines(forwarded.join('\n') + '\n');
		} catch (err: any) {
