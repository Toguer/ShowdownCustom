...
export class DexFormats {
	...
	get(name?: string | Format, isTrusted = false): Format {
		...
		if (Object.prototype.hasOwnProperty.call((this.dex.data as any).Rulesets || {}, DEFAULT_MOD + id)) {
		...
		let effect;
		if (Object.prototype.hasOwnProperty.call((this.dex.data as any).Rulesets || {}, id)) {
	...
	validPokemonTag(tagid: ID) {
		const tag = Object.prototype.hasOwnProperty.call((Tags as any) || {}, tagid) && (Tags as any)[tagid];
		if (!tag) return false;
	...
	validateBanRule(rule: string) {
		...
		for (const matchType of matchTypes) {
			...
			if (Object.prototype.hasOwnProperty.call((table as any) || {}, id)) {
		...
		if (matches.length < 1) {
			// Fallback "seguro": intenta resolver por nombre aunque el parser no haya matcheado nada.
			// Esto evita crashes cuando hay reglas tipo "+Solgaleo" y tu dex tiene la especie.
			const sp = this.dex.species.get(rule);
			if (sp?.exists) return `pokemon:${sp.id}`;
		  
			const mv = this.dex.moves.get(rule);
			if (mv?.exists) return `move:${mv.id}`;
		  
			const ab = this.dex.abilities.get(rule);
			if (ab?.exists) return `ability:${ab.id}`;
		  
			const it = this.dex.items.get(rule);
			if (it?.exists) return `item:${it.id}`;
		  
			const nat = this.dex.natures.get(rule);
			if (nat?.exists) return `nature:${nat.id}`;
