...
export class ModdedDex {
	...
	loadAliases(): NonNullable<ModdedDex['aliases']> {
		...
		const aliasTable = exported.Aliases || exported.BattleAliases || {};
		...
		for (const table of ['Items', 'Abilities', 'Moves', 'Pokedex'] as const) {
			const data = ((this.data as any)[table] || {}) as Record<string, any>;
			for (const [id, entry] of Object.entries(data) as [ID, any][]) {
				...
				const fullAcronym = fullSplit.map((x: string) => x.charAt(0)).join('');
				...
				if (spaceSplit.length !== fullSplit.length) {
					const spaceAcronym = spaceSplit.map((x: string) => x.charAt(0)).join('');
