export const Items: import('../sim/dex-items').ItemDataTable = {
	...
	flappletunite: {
		name: "Flappletunite",
		spritenum: 588,
		megaStone: ["Flapple-Mega", "Appletun-Mega"],
		megaEvolves: ["Flapple", "Appletun"],
	...
	supereviolite: {
		...
		fling: { basePower: 30 },
		onModifyAtk(atk, pokemon) {
			const s = this.dex.species.get(pokemon.species.baseSpecies);
			if (!s?.evos?.length) return;
			const ok = s.evos.some(e => this.dex.species.get(e)?.evos?.length);
			if (ok) return this.chainModify(1.5);
		},
		onModifyDef(def, pokemon) {
			const s = this.dex.species.get(pokemon.species.baseSpecies);
			if (!s?.evos?.length) return;
		...
		gen: 9,
		isNonstandard: "Future",
		shortDesc: "Si puede evolucionar dos veces: Atk/Def/SpA/SpD/Spe x1.5.",
	},
